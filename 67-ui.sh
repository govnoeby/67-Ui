...
log_folder="${XUI_LOG_FOLDER:=/var/log/67-ui}"
mkdir -p "${log_folder}"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/govnoeby/67-Ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    confirm "This function will update all 67-ui components to the latest version, and the data will not be lost. Do you want to continue?" "y"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/govnoeby/67-Ui/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    curl -fLRo /usr/bin/67-ui https://raw.githubusercontent.com/govnoeby/67-Ui/main/67-ui.sh
    chmod +x ${xui_folder}/67-ui.sh
    chmod +x /usr/bin/67-ui
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/govnoeby/67-Ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    if [[ $release == "alpine" ]]; then
        rc-service 67-ui stop
        rc-update del 67-ui
        rm /etc/init.d/67-ui -f
    else
        systemctl stop 67-ui
        systemctl disable 67-ui
        rm ${xui_service}/67-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/govnoeby/67-Ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    if [[ $twoFactorConfirm != "y" && $twoFactorConfirm != "Y" ]]; then
        ${xui_folder}/67-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor false > /dev/null 2>&1
    else
        ${xui_folder}/67-ui setting -username "${config_account}" -password "${config_password}" -resetTwoFactor true > /dev/null 2>&1
        echo -e "Two factor authentication has been disabled."
    fi

    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${green} Please use the new login username and password to access the 67-Ui panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    ${xui_folder}/67-ui setting -webBasePath "${config_webBasePath}" > /dev/null 2>&1
...
reset_config() {
    ...
    ${xui_folder}/67-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(${xui_folder}/67-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(${xui_folder}/67-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
    local URL_lists=(
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        ${xui_folder}/67-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $release == "alpine" ]]; then
            rc-service 67-ui start
        else
            systemctl start 67-ui
        fi
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            LOGI "67-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "67-ui and xray stopped successfully"
        else
...
restart() {
    if [[ $release == "alpine" ]]; then
        rc-service 67-ui restart
    else
        systemctl restart 67-ui
    fi
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "67-ui and xray Restarted successfully"
    else
...
restart_xray() {
    if [[ $release == "alpine" ]]; then
        rc-service 67-ui reload
    else
        systemctl reload 67-ui
    fi
...
status() {
    if [[ $release == "alpine" ]]; then
        rc-service 67-ui status
    else
        systemctl status 67-ui -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ $release == "alpine" ]]; then
        rc-update add 67-ui default
    else
        systemctl enable 67-ui
    fi
    if [[ $? == 0 ]]; then
        LOGI "67-ui Set to boot automatically on startup successfully"
    else
        LOGE "67-ui Failed to set Autostart"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "67-ui Autostart Cancelled successfully"
    else
        LOGE "67-ui Failed to cancel autostart"
    fi
    ...
    if [[ $release == "alpine" ]]; then
        ...
        case "$choice" in
            0)
                ...
                ;;
            1)
                journalctl -u 67-ui -e --no-pager -f -p debug
                if [[ $# == 0 ]]; then
                    before_show_menu
                fi
                ;;
...
disable_bbr() {
    ...
    if [ -f "/etc/sysctl.d/99-bbr-67-ui.conf" ]; then
        old_settings=$(head -1 /etc/sysctl.d/99-bbr-67-ui.conf | tr -d '#')
        sysctl -w net.core.default_qdisc="${old_settings%:*}"
        sysctl -w net.ipv4.tcp_congestion_control="${old_settings#*:}"
        rm /etc/sysctl.d/99-bbr-67-ui.conf
        sysctl --system
    else
...
enable_bbr() {
    ...
    if [ -d "/etc/sysctl.d/" ]; then
        ...
        } > "/etc/sysctl.d/99-bbr-67-ui.conf"
        if [ -f "/etc/sysctl.conf" ]; then
...
update_shell() {
    curl -fLRo /usr/bin/67-ui -z /usr/bin/67-ui https://github.com/govnoeby/67-Ui/raw/main/67-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ $release == "alpine" ]]; then
        ...
        if [[ $(rc-service 67-ui status | grep -F 'status: started' -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
        ...
        temp=$(systemctl status 67-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    if [[ $release == "alpine" ]]; then
        if [[ $(rc-update show | grep -F '67-ui' | grep default -c) == 1 ]]; then
            return 0
        else
            return 1
        fi
    else
        temp=$(systemctl is-enabled 67-ui)
...
ssl_cert_issue_for_ip() {
    ...
    local existing_webBasePath=$(${xui_folder}/67-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    ...
    local reloadCmd="systemctl restart 67-ui 2>/dev/null || rc-service 67-ui restart 2>/dev/null"
...
ssl_cert_issue() {
    ...
    reloadCmd="67-ui restart"

    LOGI "Default --reloadcmd for ACME is: ${yellow}67-ui restart"
    LOGI "This command will run on every certificate issue and renew."
    read -rp "Would you like to modify --reloadcmd for ACME? (y/n): " setReloadcmd
    if [[ "$setReloadcmd" == "y" || "$setReloadcmd" == "Y" ]]; then
        echo -e "\n${green}\t1.${plain} Preset: systemctl reload nginx ; 67-ui restart"
        echo -e "${green}\t2.${plain} Input your own command"
        echo -e "${green}\t0.${plain} Keep default reloadcmd"
        read -rp "Choose an option: " choice
        case "$choice" in
            1)
                LOGI "Reloadcmd is: systemctl reload nginx ; 67-ui restart"
                reloadCmd="systemctl reload nginx ; 67-ui restart"
                ;;
            2)
                LOGD "It's recommended to put 67-ui restart at the end, so it won't raise an error if other services fails"
                read -rp "Please enter your reloadcmd (example: systemctl reload nginx ; 67-ui restart): " reloadCmd
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(${xui_folder}/67-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(${xui_folder}/67-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(${xui_folder}/67-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(${xui_folder}/67-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(${xui_folder}/67-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    case "$num" in
        1)
            if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
                ...
                ${xui_folder}/67-ui setting -listenIP "${config_listenIP}" > /dev/null 2>&1
            ...
            ;;
        2)
            ${xui_folder}/67-ui setting -listenIP 0.0.0.0 > /dev/null 2>&1
...
|  ${blue}67-ui restart-xray${plain}          - Restart Xray                     │
...
│  ${blue}67-ui update-all-geofiles${plain}   - Update all geo files             │
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}67-Ui Panel Management Script${plain}                │
