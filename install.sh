...
setup_ip_certificate() {
    ...
    echo -e "${yellow}acme.sh will automatically renew and reload 67-ui before expiry.${plain}"
    return 0
}
    ...
    case "$ssl_choice" in
        ...
        4)
            ...
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ${xui_folder}/67-ui setting -listenIP "127.0.0.1" > /dev/null 2>&1
                SSL_HOST="127.0.0.1"
...
config_after_install() {
    local existing_hasDefaultCredential=$(${xui_folder}/67-ui setting -show true | grep -Eo 'hasDefaultCredential: .+' | awk '{print $2}')
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            ${xui_folder}/67-ui setting -username "${config_username}" -password "${config_password}" -port "${config_port}" -webBasePath "${config_webBasePath}"
        ...
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            ${xui_folder}/67-ui setting -username "${config_username}" -password "${config_password}"
        ...
        # Properly detect empty cert by checking if cert: line exists and has content after it
        existing_cert=$(${xui_folder}/67-ui setting -getCert true | grep 'cert:' | awk -F': ' '{print $2}' | tr -d '[:space:]')
        if [[ -z "$existing_cert" ]]; then
            echo ""
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${green}     SSL Certificate Setup (RECOMMENDED)   ${plain}"
...
install_67-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/govnoeby/67-Ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "https://api.github.com/repos/govnoeby/67-Ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
                echo -e "${red}Failed to fetch 67-ui version, it may be due to GitHub API restrictions, please try it later${plain}"
                exit 1
            fi
        fi
        echo -e "Got 67-ui latest version: ${tag_version}, beginning the installation..."
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/govnoeby/67-Ui/releases/download/${tag_version}/67-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Downloading 67-ui failed, please be sure that your server can access GitHub ${plain}"
            exit 1
        fi
    else
        tag_version=$1
        ...
        url="https://github.com/govnoeby/67-Ui/releases/download/${tag_version}/67-ui-linux-$(arch).tar.gz"
        echo -e "Beginning to install 67-ui $1"
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz ${url}
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download 67-ui $1 failed, please check if the version exists ${plain}"
            exit 1
        fi
    fi
    curl -4fLRo /usr/bin/67-ui-temp https://raw.githubusercontent.com/govnoeby/67-Ui/main/67-ui.sh
    if [[ $? -ne 0 ]]; then
        echo -e "${red}Failed to download 67-ui.sh${plain}"
        exit 1
    fi

    # Stop 67-ui service and remove old resources
    ...
    tar zxvf 67-ui-linux-$(arch).tar.gz
    rm 67-ui-linux-$(arch).tar.gz -f
    ...
    chmod +x 67-ui bin/xray-linux-$(arch)

    # Update 67-ui cli and se set permission
    mv -f /usr/bin/67-ui-temp /usr/bin/67-ui
    chmod +x /usr/bin/67-ui
    mkdir -p /var/log/67-ui
    config_after_install

    # Etckeeper compatibility
    if [ -d "/etc/.git" ]; then
        if [ -f "/etc/.gitignore" ]; then
            if ! grep -q "67-ui/67-ui.db" "/etc/.gitignore"; then
                echo "" >> "/etc/.gitignore"
                echo "67-ui/67-ui.db" >> "/etc/.gitignore"
                echo -e "${green}Added 67-ui.db to /etc/.gitignore for etckeeper${plain}"
            fi
        else
            echo "67-ui/67-ui.db" > "/etc/.gitignore"
            echo -e "${green}Created /etc/.gitignore and added 67-ui.db for etckeeper${plain}"
        fi
    fi

    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/67-ui https://raw.githubusercontent.com/govnoeby/67-Ui/main/67-ui.rc
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Failed to download 67-ui.rc${plain}"
            exit 1
        fi
        chmod +x /etc/init.d/67-ui
        rc-update add 67-ui
        ...
        if [ -f "67-ui.service" ]; then
            echo -e "${green}Found 67-ui.service in extracted files, installing...${plain}"
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "67-ui.service.debian" ]; then
                        echo -e "${green}Found 67-ui.service.debian in extracted files, installing...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "67-ui.service.arch" ]; then
                        echo -e "${green}Found 67-ui.service.arch in extracted files, installing...${plain}"
                ...
                *)
                    if [ -f "67-ui.service.rhel" ]; then
                        echo -e "${green}Found 67-ui.service.rhel in extracted files, installing...${plain}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/67-ui.service https://raw.githubusercontent.com/govnoeby/67-Ui/main/67-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/67-ui.service https://raw.githubusercontent.com/govnoeby/67-Ui/main/67-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/67-ui.service https://raw.githubusercontent.com/govnoeby/67-Ui/main/67-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
        ...
        if [ "$service_installed" = true ]; then
            ...
            echo -e "${red}Failed to install 67-ui.service file${plain}"
            exit 1
        fi
    fi

    echo -e "${green}67-ui ${tag_version}${plain} installation finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
